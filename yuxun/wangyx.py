...
def isTriangle(tri_v):
    if not (len(tri_v) == 3):
        ...
        return (distance(tri_v[0], tri_v[1]) + distance(tri_v[1], tri_v[2]) > distance(tri_v[0], tri_v[2])) and \
               (distance(tri_v[1], tri_v[2]) + distance(tri_v[2], tri_v[0]) > distance(tri_v[0], tri_v[1])) and \
               (distance(tri_v[2], tri_v[0]) + distance(tri_v[0], tri_v[1]) > distance(tri_v[1], tri_v[2]))
...
def getCrossoverPoint(p1, p2, p3, p4):
    if(p2[0] - p1[0]==0) and (p4[0] - p3[0]==0):   # 无交点
        return [False, 0]
...
def getTriBrightness(tri_v, p_b):
    ...
    for i in range(x_l[0], x_l[2]+1):
        for j in range(y_l[0], y_l[2]+1):
            ...
            if mutiPly(tri_v[0], tri_v[1], [i, j]) * mutiPly(tri_v[1], tri_v[2], [i, j]) >= 0 and \
                    mutiPly(tri_v[0], tri_v[1], [i, j]) * mutiPly(tri_v[0], tri_v[2], [i, j]) >= 0:
                num_p += 1
                brightness += p_b[i][j]
...
def getAreaBrightnessx(v_set, url):
    ...
    :param v_set: list,vertex of the convex multi deformation
    :param url: url of the picture
    :return: 区域面积和区域平均亮度（像素值）
    """
...
def getAreaBrightness(v_set, image):
    """
    获得区域面积和区域平均亮度（像素值）
    :param v_set: 构成指定区域的点集（6L,2L）的矩阵
    :param image: 原图
    :return: 区域面积和区域平均亮度（像素值）
    """
    Area = 0
    ...
    while len(v_dict.keys()) > 3:
        ...
        if tri[0]:
            ...
            print ("error,it's not a multiple deformations")
            return 0
